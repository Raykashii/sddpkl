...
public class MusergroupmenuDAO {
	...
	public List<Musergroupmenu> listPaging(int first, int second, String filter, String orderby) throws Exception {
		...
		oList = session.createSQLQuery("select * from Musergroupmenu where " + filter + " order by " + orderby
				+ " offset " + first + " rows fetch next " + second + " rows only").addEntity(Musergroupmenu.class)
				.list();
	...
	public int pageCount(String filter) throws Exception {
		...
		if (filter == null || "".equals(filter))
			filter = "0 = 0";
		session = StoreHibernateUtil.openSession();
		count = Integer.parseInt((String) session.createSQLQuery("select count(*) from Musergroupmenu where " + filter)
	...
	@SuppressWarnings("unchecked")
	public List<Musergroupmenu> listByMusergroupfk(Integer musergroupfk) throws Exception {
		List<Musergroupmenu> oList = null;
		session = StoreHibernateUtil.openSession();
		oList = session.createNativeQuery("SELECT * FROM MUSERGROUPMENU WHERE MUSERGROUPFK = " + musergroupfk)
	...
	public Musergroupmenu findByPk(Integer pk) throws Exception {
		session = StoreHibernateUtil.openSession();
		Musergroupmenu oForm = (Musergroupmenu) session
				.createQuery("from Musergroupmenu where Musergroupmenupk = " + pk).uniqueResult();
		session.close();
		return oForm;
	}
